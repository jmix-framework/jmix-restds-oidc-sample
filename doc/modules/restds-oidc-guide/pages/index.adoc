...
. To use the OAuth2 token of the current user when making requests to the Service's REST API, the Client application defines the following properties for the `serviceapp` REST DataStore:
+
[source,properties,indent=0]
.integrated-apps/client-app/src/main/resources/application.properties
----
...
. To provide the database persistence for UI filter configurations and user settings, the following two entities are copied from the framework's `jmix-flowui-data` module:
+
[source,java,indent=0]
.separate-tiers/backend-app/src/main/java/com/company/backendapp/entity/uidata/FilterConfiguration.java
----
include::example$/separate-tiers/backend-app/src/main/java/com/company/backendapp/entity/uidata/FilterConfiguration.java[tags=entity]
----
+
[source,java,indent=0]
.separate-tiers/backend-app/src/main/java/com/company/backendapp/entity/uidata/UserSettingsItem.java
----
include::example$/separate-tiers/backend-app/src/main/java/com/company/backendapp/entity/uidata/UserSettingsItem.java[tags=entity]
----
...
+
[source,java,indent=0]
.separate-tiers/frontend-app/src/main/java/com/company/frontendapp/security/FrontendUserMapper.java
----
...
. To use the OAuth2 token of the current user when making requests to the Backend REST API, the Frontend application defines the following properties for the `backend` REST DataStore:
+
[source,properties,indent=0]
.separate-tiers/frontend-app/src/main/resources/application.properties
----
